...
annotated_panel_plot <- function(plots) {
  if (length(plots) != 10) {
    stop("You must supply a list of exactly 10 plots.")
  }
  ...
  plot_grid <- (plots[[1]] | plots[[6]] | plots[[2]] | plots[[7]]) /
               (plots[[3]] | plots[[8]] | plots[[9]] | plots[[10]])
...
df1 <- df1 %>% mutate(
  log_change_true_kp_adj = rttm.adj(log_init_true, log_final_true)[, 3],
  log_change_obs_kp_adj = rttm.adj(log_init_obs, log_final_obs)[, 3]
)

# Apply Blomqvist correction
k <- v_err / (v_pop + v_err)
b_blomq <- (coef(lm(log_change_obs ~ log_init_obs, data = df1))[2] + k) / (1 - k)
df1 <- df1 %>% mutate(
  log_change_true_blomq_adj = blomqvist_pseudo_exact(log_init_true, log_final_true, b_blomq),
  log_change_obs_blomq_adj = blomqvist_pseudo_exact(log_init_obs, log_final_obs, b_blomq)
)
...
# Plot 4: KP-ADJUSTED TRUE change vs. initial scatterplot
#p4 <- p3 + aes(x = log_init_true, y = log_change_true_kp_adj)

# Plot 5: BLOM-adjusted TRUE change vs. initial scatterplot
#p5 <- p3 + aes(x = log_init_true, y = log_change_true_blomq_adj)

# Plot 6: OBSERVED Chiolero style
p6 <- chiolero_plot(df1, log_init_obs, log_final_obs)

# Plot 7: OBSERVED final vs. initial scatterplot
p7 <- p2 + aes(x = log_init_obs, y = log_final_obs)

# Plot 8: OBSERVED change vs. initial scatterplot
p8 <- p3 + aes(x = log_init_obs, y = log_change_obs)

# Plot 9: KP-ADJUSTED OBSERVED change vs. initial scatterplot
p9 <- p4 + aes(x = log_init_obs, y = log_change_obs_kp_adj)

# Plot 10: BLOM-ADJUSTED change vs. initial scatterplot
p10 <- p4 + aes(x = log_init_obs, y = log_change_obs_blomq_adj)

plots1 <- list(p1, p2, p3, plot.new(), plot.new(), p6, p7, p8, p9, p10)
...
test.cors <- function(v_pop, v_ind, v_err, b, n = 1000) {
  ...
  bind_rows(.id = "cor.type",
        obs.b = tidy(lm(log_change_obs ~ log_init_obs, data = df))[2,],
        #obs.b2 = tidy(lm(log_change_obs ~ scale((log_init_obs+log_init_obs2)/2), data = df))[2,],
        adj.b = tidy(lm(log_change_obs_adj ~ log_init_obs, data = df))[2,],
...
pdam_mod <- lm(log_change_obs ~ log_init_obs, data = pdam)
pdam_b_obs <- coef(pdam_mod)[[2]]      # -0.866 (observed slope)
...
pdam_var_x_rel <- pdam_var_x_abs / pdam_v_pop_abs
...
pdam_mod_adj <- lm(log_change_obs_adj ~ log_init_obs, data = pdam)
...
k <- pdam_v_err_abs / pdam_var_x_abs 
...
blomqvist_boot_scaled <- function(data, indices) {
  d <- data[indices, ]
  b_obs_i <- coef(lm(log_change_obs ~ log_init_obs, data = d))[2]
...
mill_mod <- lm(log_change_obs ~ log_init_obs, data = mill)  # scaled to unit variance to match simulations
mill_b_obs <- coef(mill_mod)[[2]]      # observed slope
...
# Kelly-Price adjusted point estimate and CI
mill_mod_adj <- lm(log_change_obs_adj ~ log_init_obs, data = mill)
...
evaluate_methods <- function(b_true, v_ind, v_err, v_pop, reps, n, nsim, seed) {
  replicate_results <- purrr::map_dfr(seq_len(reps), function(i) {
    ...
    kp_fit <- lm(log_change_obs_adj ~ log_init_obs, data = syn_df)
    b_kp <- coef(kp_fit)[[2]]
    ...
    #   change <- b * x + rnorm(nsim, 0, sqrt(v_ind))
    #   final <- x + change
...
pdam_v_err_abs
pdam_v_pop_abs
pdam_v_err_abs / (pdam_var_x_abs - pdam_v_err_abs)

test_syn_eval <- evaluate_methods(
  b_true = -0.3,
  v_ind = 0.1404,      # pdam_v_ind_est_abs
  v_err = 0.0231,   # pdam_v_err_abs
  v_pop = 0.1608,   # pdam_v_pop_abs
...
#results_long$v_ind <- factor(results_long$v_ind, levels = rev(sort(unique(results_long$v_ind))))
