...
panel_plot_row7 <- function(plots) {
  ...
  myPlots <- cowplot::plot_grid(text_panel(b, v_ind, v_err, base_size = 8),
                                ...
                                rel_widths = c(0.5, 0.7, 0.7, 1, 1, 1, 1, 1))
  return(myPlots)
}
...
run_simulation_scenario <- function(
  ...
  df1 <- df1 %>%
    dplyr::mutate(
      log_change_kp_adj    = kp_full$adj.growth,
      log_change_blomq_adj = blomqvist_pseudo_exact(log_init_obs, log_final_obs, b_blomq_adj),
      log_change_mom_adj   = moment.nohet_psuedo_exact(log_init_obs, log_final_obs, b_mom_adj)
    )
  ...
  slopes_tbl <- tibble::tibble(
    method   = factor(c("Obs", "K-P", "Blomq", "Mom"), levels = c("Obs", "K-P", "Blomq", "Mom")),
...
header <- make_column_header(
  labels = c("1. Parameters", "2. True\nvalues", "3. Observed\nvalues", 
             "4. Observed", "5. Kelly-Price", "6. Blomqvist", "7. Moments", "8. Slope\ncomparison"),
  rel_widths = c(0.7, 0.5, 0.5, rep(1, 5)),      # <- use the SAME rel_widths you used for each row
  base_size = 10
)
