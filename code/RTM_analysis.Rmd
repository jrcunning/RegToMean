---
title: "Analysis code for: Failure to distinguish between measurement error and variation in individual change leads to improper correction of regression-to-the-mean artifacts"
date: "December 2025"
output:
  html_document:
    toc: true
...
panel_plot_row7 <- function(plots) {
  ...
  myPlots <- cowplot::plot_grid(text_panel(b, v_ind, v_err, base_size = 8),
                                ...
                                rel_widths = c(0.6, 0.6, 0.6, 1, 1, 1, 1, 1))
  return(myPlots)
}
...
# combine header + 4 rows
# (Set rel_widths to match each row of 7 panels)
header <- make_column_header(
  labels = c("1. Param-\neters", "2. True\nvalues", "3. Observed\nvalues", 
             "4. Observed\nbaseline effect", "5. Kelly–Price\nadjusted", "6. Blomqvist\nadjusted", "7. Moments\nadjusted", "8. Slope\ncomparison"),
  rel_widths = c(0.6, 0.6, 0.6, rep(1, 5)),      # <-  SAME rel_widths used for each row
  base_size = 10
)

footer <- make_column_header(
  labels = c("", " timepoint", " timepoint", "initial", "initial", "initial", "initial", " method"),
  rel_widths = c(0.6, 0.6, 0.6, rep(1, 5)),
  base_size = 8, face = "plain", vjust = -0.25
)

fig1 <- cowplot::plot_grid(
  header,
  p1, p2, p4, p5,              # four 7-panel rows
  footer,
  labels = c("", "A", "B", "C", "D", ""),
  ncol = 1,
  rel_heights = c(0.25, 1, 1, 1, 1, 0.02)  # give the header some breathing room
)
fig1

ggsave(filename = "figures/fig1.png", width = 200, height = 140, units = "mm", bg = "white")
```
...
ggplot(results_long, aes(
  ...
  color = method,
  group = method,
  shape = factor(ifelse(error > 0.75, 2, ifelse(error < -0.75, 6, 16)))
)) +
...
ggsave(filename = "figures/fig2.png", width = 160, height = 120, units = "mm", bg = "white")
...
```{r}
# Alternate version of Figure 2 that shows slopes instead of slope biases
...
ggsave(filename = "figures/fig2alt.png", width = 160, height = 120, units = "mm", bg = "white")
```
...
# Normalize bleaching response by symbiont (intercept shift)
## Confirm that baseline-change relationship (slope) does not vary by symbiont
pdam_mod1 <- lm(log_change_raw ~ log_init_raw * sym, data = pdam)
anova(pdam_mod1)    # no significant interaction => same slope
pdam_mod2 <- lm(log_change_raw ~ log_init_raw + sym, data = pdam)
summary(pdam_mod2)  # observed slope is -0.8664. C and D have diff intercepts.
## Shift final measurements of C and D corals by group intercepts, and
## maintain global mean change values. 
p_C <- mean(pdam$sym == "C")   # proportion of colonies with C
p_D <- mean(pdam$sym == "D")   # proportion of colonies with D
gamma_D <- coef(pdam_mod2)["symD"]     # D - C intercept difference
pdam <- pdam %>%
  mutate(
    shift = if_else(sym == "C",
                    p_D * gamma_D,    # shift C up
                    -p_C * gamma_D),  # shift D down
    log_change_symnorm = log_change_raw + shift,
    log_final_symnorm  = log_init_raw + log_change_symnorm
  )
# Confirm that this shift did not alter the baseline-change slope
pdam_mod3 <- lm(log_change_symnorm ~ log_init_raw, data = pdam)
summary(pdam_mod3)    # slope is still -0.8664
# Confirm that shift did not alter the global mean symbiont loss
mean(pdam$log_change_raw)    # raw change values
mean(pdam$log_change_symnorm)    # shifted change values
var(pdam$log_change_raw)
var(pdam$log_change_symnorm)

# Plot raw and shifted values to demonstrate normalization
p1 <- ggplot(pdam, aes(x = log_init_raw, y = log_change_raw)) +
  ...
  stat_regline_equation(aes(group = sym, color = sym)) +
  coord_cartesian(xlim = c(-3.3, -0.9), ylim = c(-3.1, 0.6))

p2 <- ggplot(pdam, aes(x = log_init_raw, y = log_change_symnorm)) +
  geom_point(aes(color = sym)) +
...
#          log_init_obs = log_init_raw,
#          log_final_obs = grand_mean_final + group_resid_final,
#          log_change_obs = log_final_obs - log_init_obs) %>%
#   select(colony, sym, log_init_obs, log_final_obs, log_change_obs, log_change_raw, log_init_raw, log_final_raw)
# var(pdam$log_change_raw)
# var(pdam$log_change_obs)
# 
# pdam_mod1 <- lm(log_change_raw ~ log_init_raw + sym, data = pdam)
# summary(pdam_mod1)
# pdam_mod2 <- lm(log_change_obs ~ log_init_obs, data = pdam)
# summary(pdam_mod2)
# pdam_mod3 <- lm(log_change_obs ~ log_init_obs + sym, data = pdam)
# summary(pdam_mod3)
# 
# 
# p1 <- ggplot(pdam, aes(x = log_init_raw, y = log_change_raw)) +
...
pdam_var_x_abs <- var(pdam$log_init_raw, na.rm = TRUE)
pdam_var_x_abs

# Blomqvist's k (1 - R) from time-series study (see "pdam_variance.R")
pdam_k <- 0.4927648 # <- 1 - Repeatability from warming dataset rptR
pdam_k <- 0.602  # rough calc. 
pdam_k <- 0.467
...
pdam_fit_obs <- lm(log_change_symnorm ~ log_init_raw, data = pdam)
...
pdam_b_obs_boot <- boot::boot(
  ...
  statistic = function(d, i) {
    di <- d[i, , drop = FALSE]
    coef(lm(di$log_change_symnorm ~ di$log_init_raw))[2]
  },
  R = 2000
)
...
# --- 3) Kelly–Price (point + bootstrap CI) ---
pdam_kp_full <- rttm.adj(pdam$log_init_raw, pdam$log_final_symnorm)
pdam_b_kp_adj <- coef(lm(pdam_kp_full$adj.growth ~ pdam$log_init_raw))[2]
...
pdam_kp_boot <- boot::boot(
  ...
  statistic = function(d, i) {
    di <- d[i, , drop = FALSE]
    kp <- rttm.adj(di$log_init_raw, di$log_final_symnorm)
    coef(lm(kp$adj.growth ~ di$log_init_raw))[2]
  },
  R = 2000
)
...
pdam_v_ind <- var(pdam$log_change_symnorm) - (2 * pdam_v_err_abs) - (pdam_b_blomq_adj^2 * pdam_v_pop)
pdam_v_ind
...
pdam <- pdam %>%
  dplyr::mutate(
    ...
    log_change_blomq_adj = blomqvist_pseudo_exact(log_init_raw, log_change_symnorm, pdam_b_blomq_adj)
  )
...
pdam_blomqvist_boot <- boot::boot(
  ...
  statistic = function(d, i) {
    di <- d[i, , drop = FALSE]
    b_i <- coef(lm(log_change_symnorm ~ log_init_raw, data = di))[2]
...
# --- 5) Method of moments (point + bootstrap CI) ---
pdam_b_mom_adj <- moment.nohet.safe(pdam$log_init_raw, pdam$log_final_symnorm)

  # pseudo-exact y’ (used only for plotting panel, not for CI)
  pdam <- pdam %>%
    dplyr::mutate(
      log_change_mom_adj   = moment.nohet_psuedo_exact(log_init_raw, log_change_symnorm, pdam_b_mom_adj)
    )
  ...
  pdam_mom_boot <- boot::boot(
    ...
    statistic = function(d, i) {
      di <- d[i, , drop = FALSE]
      b_i <- moment.nohet.safe(di$log_init_raw, di$log_final_symnorm)
    },
    R = 2000
  )
...
pdam_trait <- range(c(pdam$log_init_raw, pdam$log_final_symnorm), na.rm = TRUE)
pdam_change <- range(pdam$log_change_symnorm, na.rm = TRUE)
pdam_theta <- sd(pdam$log_final_symnorm) / sd(pdam$log_init_raw)
pdam_theta

pdam_chiolero_plot <- chiolero_plot(pdam, log_init_raw, log_final_symnorm) + 
...
pdam_initfinal_plot <- ggplot(pdam, aes(x = log_init_raw, y = log_final_symnorm, color = sym)) +
    ...
    ggtitle("Observed")
pdam_initchange_plot <- ggplot(pdam, aes(x = log_init_raw, y = log_change_symnorm)) +
    geom_point(alpha = 0.2) +
    stat_cor(aes(label = ..p.label..), size = 2,
                     label.x = -2.25, label.y = Inf, hjust = 0.5, vjust = 1) +
    geom_smooth(method = "lm", se = FALSE) +
    coord_cartesian(xlim = c(-3.2, -1.0), 
                    ylim = c(pdam_change[1], pdam_change[2]+0.1)) +
    labs(title = NULL, x = NULL, y = "change") +
    ggplot2::theme_minimal(base_size = 8)+
    theme(plot.margin = margin(1,0,1,0),
          axis.title.y = element_text(margin = margin(r = 0)))
pdam_kp_plot <- ggplot(pdam, aes(x = log_init_raw, y = log_change_kp_adj)) +
    geom_point(alpha = 0.2) +
    stat_cor(aes(label = ..p.label..), size = 2,
                     label.x = -2.25, label.y = Inf, hjust = 0.5, vjust = 1) +
    geom_smooth(method = "lm", se = FALSE) +
    coord_cartesian(xlim = c(-3.2, -1.0), ylim = c(pdam_change[1], pdam_change[2]+0.1)) +
    labs(title = NULL, x = NULL, y = "change") +
    ggplot2::theme_minimal(base_size = 8)+
    theme(plot.margin = margin(1,0,1,0),
          axis.title.y = element_text(margin = margin(r = 0)))
pdam_blomq_plot <- ggplot(pdam, aes(x = log_init_raw, y = log_change_blomq_adj)) +
    geom_point(alpha = 0.2) +
    stat_cor(aes(label = ..p.label..), size = 2,
                     label.x = -2.25, label.y = Inf, hjust = 0.5, vjust = 1) +
    geom_smooth(method = "lm", se = FALSE) +
    ...
    theme(plot.margin = margin(1,0,1,0),
          axis.title.y = element_text(margin = margin(r = 0)))
pdam_mom_plot <- ggplot(pdam, aes(x = log_init_raw, y = log_change_mom_adj)) +
    geom_point(alpha = 0.2) +
    stat_cor(aes(label = ..p.label..), size = 2,
                     label.x = -2.25, label.y = Inf, hjust = 0.5, vjust = 1) +
    geom_smooth(method = "lm", se = FALSE) +
...
pdam_plots <- cowplot::plot_grid(
  text_panel_math(
    ...
    line6 = sprintf("s[Delta[ind]]^2==bold(%s)", formatC(pdam_v_ind, format = "fg", digits = 3)),
...
mill_plots <- cowplot::plot_grid(
  text_panel_math(
    ...
    line6 = sprintf("s[Delta[ind]]^2==bold(%s)", formatC(mill_v_ind, format = "fg", digits = 3)),
...
# (Set rel_widths to match each row of 7 panels)
header <- make_column_header(
  labels = c("1. Dataset       \n ", "2. Observed\nvalues", "3. Observed\nbaseline effect", "4. Kelly-Price\nadjusted", 
             "5. Blomqvist\nadjusted", "6. Moments\nadjusted", "7. Slope\ncomparison"),
  rel_widths = c(1, 0.6, 1, 1, 1, 1, 1),      # <- SAME rel_widths used for each row
  base_size = 10
)
footer <- make_column_header(
  labels = c("", "timepoint", "initial", "initial", "initial", "initial", " method"),
  rel_widths = c(1, 0.6, 1, 1, 1, 1, 1),
  base_size = 8, face = "plain", vjust = -0.2
)

fig3 <- cowplot::plot_grid(
  header, pdam_plots, mill_plots, footer, align = "hv", 
  ncol = 1, labels = c("", "A", "B", ""),
  rel_heights = c(0.25, 1, 1, 0.02)  
)
fig3

ggsave(filename = "figures/fig3.png", width = 200, height = 80, units = "mm", bg = "white")
```
