...
header <- make_column_header(
  ...
  rel_widths = c(0.50, 0.65, 0.65, rep(1, 5)),      # <-  SAME rel_widths used for each row
  base_size = txt_med_pt
)

footer <- make_column_header(
  labels = c("", " timepoint", " timepoint", "initial", "initial", "initial", "initial", " method"),
  rel_widths = c(0.50, 0.65, 0.65, rep(1, 5)),
  base_size = txt_small_pt, face = "plain", vjust = -0.25
)
...
fig2 <- cowplot::plot_grid(
  ...
  footer,
  labels = c("", "A", "B", "C", "D", ""),
  label_size = txt_large_pt,
  ncol = 1,
  rel_heights = c(0.25, 1, 1, 1, 1, 0.02)  # give the header some breathing room
)
fig2

ggsave(filename = "figures/fig2.png", width = 17, height = 11.9, units = "cm", bg = "white")
```
...
header <- make_column_header(
  labels = c("1. Dataset       \n ", "2. Observed\nvalues", "3. Observed\nbaseline effect", "4. Kelly-Price\nadjusted", 
             "5. Blomqvist\nadjusted", "6. Moments\nadjusted", "7. Slope\ncomparison"),
  rel_widths = c(1.05, 0.575, 0.975, 1, 1, 1, 1),      # <- SAME rel_widths used for each row
  base_size = txt_med_pt
)
footer <- make_column_header(
  labels = c("", "timepoint", "initial", "initial", "initial", "initial", " method"),
  rel_widths = c(1.05, 0.575, 0.975, 1, 1, 1, 1),
  base_size = txt_small_pt, face = "plain", vjust = -0.2
)
...
ggsave(filename = "figures/fig4.png", width = 17, height = 6.8, units = "cm", bg = "white")
```
