...
df <- s1.1_results$data
b_obs <- s1.1_results$slopes$estimate[1]; b_obs
bfi <- coef(lm(log_final_obs ~ log_init_obs, data = df))[[2]]; bfi
r <- cor(df$log_final_obs, df$log_init_obs); r
r2 <- 2*r*sd(df$log_final_obs)*sd(df$log_init_obs) / (var(df$log_final_obs) + var(df$log_init_obs))
v <- sd(df$log_final_obs) / sd(df$log_init_obs); v
r * v   # equivalent to bfi
(1 - r * v) * -1  # equivalent to b_obs (* -1 because KP framed as m1-m2 instead of m2-m1)
r * v - 1 # also equivalent ^^
b_obs + (1 - r2)  # equivalent to b_kp_adj
r2 - 1
b_kp <- s1.1_results$slopes$estimate[2]; b_kp
r * v - r2
...
# test variance of blomq-adjusted points
var(s1.1_results$data$log_change_obs)
var(s1.1_results$data$log_change_kp_adj)
var(s1.1_results$data$log_change_blomq_adj)
```
...
df <- s1.2_results$data
b_obs <- s1.2_results$slopes$estimate[1]; b_obs
bfi <- coef(lm(log_final_obs ~ log_init_obs, data = df))[[2]]; bfi
r <- cor(df$log_final_obs, df$log_init_obs); r
r2 <- 2*r*sd(df$log_final_obs)*sd(df$log_init_obs) / (var(df$log_final_obs) + var(df$log_init_obs))
v <- sd(df$log_final_obs) / sd(df$log_init_obs); v
...
(1 - r * v) * -1  # equivalent to b_obs (* -1 because KP framed as m1-m2 instead of m2-m1)
r * v - 1
b_obs + (1 - r)  # equivalent to b_kp_adj
b_kp <- s1.2_results$slopes$estimate[2]; b_kp
...
header <- make_column_header(
  ...
  rel_widths = c(0.6, 0.6, 0.6, rep(1, 5)),      # <- use the SAME rel_widths you used for each row
  base_size = 10
)
...
pdam <- pdam %>%
  ...
  mutate(log_init_obs = grand_mean_init + group_resid_init,
         ...
         log_change_obs = log_final_obs - log_init_obs) %>%
  select(colony, sym, log_init_obs, log_final_obs, log_change_obs, log_init_raw, log_final_raw, log_change_raw)

# Get pdam variances
# Total variance in initial observed trait values
pdam_var_x_abs <- var(pdam$log_init_obs, na.rm = TRUE)
var(pdam$log_init_obs, na.rm = TRUE)
var(pdam$log_init_raw, na.rm = TRUE)
...
#non-sym-adjusted
pdam_fit_obs <- lm(log_change_raw ~ log_init_raw + sym, data = pdam)
pdam_b_obs   <- coef(pdam_fit_obs)[2]
...
pdam_k2 <- pdam_v_err_abs2 / pdam_var_x_abs
pdam_k2 <- 0.497 # 1 - repeatability from warming study
pdam_b_blomq_adj <- blomqvist_b(pdam_b_obs, pdam_k)
pdam_b_blomq_adj2 <- blomqvist_b(pdam_b_obs, pdam_k2)
...
pdam_v_ind2 <- var(pdam$log_change_obs) - (2 * pdam_v_err_abs2) - (pdam_b_blomq_adj2^2 * pdam_v_pop2)
...
pdam <- pdam %>%
  dplyr::mutate(
    ...
    log_change_blomq_adj2 = blomqvist_pseudo_exact(log_init_obs, log_change_obs, pdam_b_blomq_adj2)
  )
...
pdam_blomqvist2_boot <- boot::boot(
  data = pdam,
  statistic = function(d, i) {
    di <- d[i, , drop = FALSE]
    b_i <- coef(lm(log_change_obs ~ log_init_obs, data = di))[2]
    #k_i <- pdam_v_err_abs2 / var(di$log_init_obs)
    blomqvist_b(b_i, pdam_k2)
  },
  R = 2000
)
pdam_b_blomq_adj2_confint_boot <- stats::quantile(pdam_blomqvist2_boot$t, c(0.025, 0.975), na.rm = TRUE)
...
pdam_slopes_tbl <- tibble::tibble(
  method   = factor(c("Obs.", "K.–P.", "Blom.", "Mom."), levels = c("Obs.", "K.–P.", "Blom.", "Mom.")),
  estimate = c(pdam_b_obs, pdam_b_kp_adj, pdam_b_blomq_adj2, pdam_b_mom_adj),
  ci_lower = c(pdam_b_obs_confint_boot[1], pdam_b_kp_adj_confint_boot[1], pdam_b_blomq_adj2_confint_boot[1], pdam_b_mom_adj_confint_boot[1]),
  ci_upper = c(pdam_b_obs_confint_boot[2], pdam_b_kp_adj_confint_boot[2], pdam_b_blomq_adj2_confint_boot[2], pdam_b_mom_adj_confint_boot[2])
)
...
pdam_kp_plot <- ggplot(pdam, aes(x = log_init_obs, y = log_change_kp_adj)) +
    ...
    theme(plot.margin = margin(1,0,1,0),
          axis.title.y = element_text(margin = margin(r = 0)))
pdam_blomq_plot <- ggplot(pdam, aes(x = log_init_obs, y = log_change_blomq_adj2)) +
    geom_point(alpha = 0.2) +
    stat_cor(aes(label = ..p.label..), size = 2,
                     label.x = -2.25, label.y = Inf, hjust = 0.5, vjust = 1) +
    geom_smooth(method = "lm", se = FALSE) +
...
pdam_plots <- cowplot::plot_grid(
  text_panel_math(
    ...
    line5 = sprintf("s[X]^2==bold(%s)", formatC(pdam_var_x_abs, format = "fg", digits = 3)),
    line6 = sprintf("s[Delta[ind]]^2==bold(%s)", formatC(pdam_v_ind2, format = "fg", digits = 3)),
    line7 = sprintf("s[err]^2==bold(%s)", formatC(pdam_v_err_abs2, format = "fg", digits = 3))
  ),
...
# Something from Tu and Gilthorpe
...
```{r mill_import_tidy}
# Import data
mill <- read_csv("data/subMil1.csv")       # Generated in Carly's script
...
mill_plots <- cowplot::plot_grid(
  text_panel_math(
    ...
    line5 = sprintf("s[X]^2==bold(%s)", formatC(mill_var_x_abs, format = "fg", digits = 3)),
...
header <- make_column_header(
  labels = c("1. Dataset       \n ", "2. Observed\nvalues", "3. Observed\nbaseline effect", "4. Kelly-Price\nadjusted", 
             "5. Blomqvist\nadjusted", "6. Moments\nadjusted", "7. Slope\ncomparison"),
  rel_widths = c(1, 0.6, 1, 1, 1, 1, 1),      # <- use the SAME rel_widths you used for each row
  base_size = 10
)
footer <- make_column_header(
...
fig3 <- cowplot::plot_grid(
  ...
  ncol = 1, labels = c("", "A", "B", ""),
  rel_heights = c(0.25, 1, 1, 0.02)  # give the header some breathing room
)
fig3
