...
chiolero_plot <- function(df, init_var, final_var, label = NULL) {
  ...
  ggplot(df, aes(x = 1, xend = 2, y = !!init_sym, yend = !!final_sym)) +
    ...
    geom_point(aes(x = 1, y = !!init_sym), size = 1.5) +
    geom_point(aes(x = 2, y = !!final_sym), size = 1.5) +
    scale_y_continuous(limits = c(-max_abs_initfinal, max_abs_initfinal)) +
...
p2 <- ggplot(df1, aes(x = log_init_true, y = log_final_true)) +
  ...
  stat_cor(label.x = -max_abs_initfinal, label.y = max_abs_initfinal) +
  geom_smooth(method = "lm", se = FALSE) +
  #stat_regline_equation(aes(label = ..eq.label..),
  #                     label.y.npc = 0.1, output.type = "text") +
  # stat_var_test(label.x = max_abs_initfinal, label.y = -max_abs_initfinal, 
  #               hjust = 1, vjust = 0) +
...
p3 <- ggplot(df1, aes(x = log_init_true, y = log_change_true)) +
  ...
  stat_cor(label.x = -max_abs_initchange, label.y = max_abs_initchange) +
  geom_smooth(method = "lm", se = FALSE) +
  #stat_regline_equation(aes(label = ..eq.label..),
  #                     label.y.npc = 0.1, output.type = "text") +
...
# ## Compute plot limit values
max_abs_initfinal <- df2 %>%
  select(log_init_true, log_final_true, log_init_obs, log_final_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
max_abs_initchange <- df2 %>%
  select(log_init_true, log_change_true, log_init_obs, log_change_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
...
# ## Compute plot limit values
max_abs_initfinal <- df3 %>%
  select(log_init_true, log_final_true, log_init_obs, log_final_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
max_abs_initchange <- df3 %>%
  select(log_init_true, log_change_true, log_init_obs, log_change_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
...
# ## Compute plot limit values
max_abs_initfinal <- df4 %>%
  select(log_init_true, log_final_true, log_init_obs, log_final_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
max_abs_initchange <- df4 %>%
  select(log_init_true, log_change_true, log_init_obs, log_change_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
...
# ## Compute plot limit values
max_abs_initfinal <- df5 %>%
  select(log_init_true, log_final_true, log_init_obs, log_final_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
max_abs_initchange <- df5 %>%
  select(log_init_true, log_change_true, log_init_obs, log_change_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
...
# ## Compute plot limit values
max_abs_initfinal <- df6 %>%
  select(log_init_true, log_final_true, log_init_obs, log_final_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
max_abs_initchange <- df6 %>%
  select(log_init_true, log_change_true, log_init_obs, log_change_obs) %>%
  summarise(max_abs = max(abs(c_across(everything())), na.rm = TRUE)) %>%
  pull(max_abs)
...
results %>% 
  ...
  ggplot(aes(x = v_ind, y = v_err)) +
    ...
    facet_wrap(~b, ncol = 5, labeller = labeller(b = function(x) paste0("b = ", x))) +
...
```{r}
slopes <- filter(results, cor.type %in% c("adj.b", "obs.b"))

# Measurement error variance
ggplot(slopes, aes(x = b, y = estimate)) +
  geom_jitter(aes(color = v_err), width = 0.05, height = 0, alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~cor.type) +
  geom_hline(yintercept = c(-0.5096655, -0.2335527), linetype = "dotted")

# Individual response variance
ggplot(slopes, aes(x = b, y = estimate)) +
  geom_jitter(aes(color = v_ind), width = 0.05, height = 0, alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~cor.type) +
  geom_hline(yintercept = c(-0.5096655, -0.2335527), linetype = "dotted")

# Error/response variance
ggplot(slopes, aes(x = b, y = estimate)) +
  geom_jitter(aes(color = log(v_err/v_ind), shape = p.value < 0.01), 
              width = 0.05, height = 0, alpha = 0.2, size = 3) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~cor.type) +
  scale_color_gradient2(low = "#2b83ba", mid = "#ffffbf", high = "#d7191c", midpoint = 0) +
  geom_hline(yintercept = c(-0.5096655, -0.2335527), linetype = "dotted") +
  labs(x = "True slope", y = "Observed slope") +
  theme_minimal()
```
...
var(pdam$log_init_obs)
var(pdam$log_final_obs)

pdam %>%
  pivot_longer(c(log_init_obs, log_final_obs)) %>%
  ggplot(aes(x = name, y = value, group = factor(colony), color = factor(colony))) +
  geom_point() +
  geom_line()
```
...
pdam_plots <- list(
  p5 %+% pdam,
  p6 %+% pdam + xlim(-4.1, -1) + ylim(-4.1, -1),
  p7 %+% pdam + xlim(-3.25, -1) + ylim(-2.5, 0.5),
  p8 %+% pdam + xlim(-3.25, -1) + ylim(-2.5, 0.5)
)
...
mill_plots <- list(
  p5 %+% mill,
  p6 %+% mill + xlim(0.5, 4.5) + ylim(-0.5, 5),
  p7 %+% mill + xlim(0.5, 4.5) + ylim(-2.2, 1.8),
  p8 %+% mill + xlim(0.5, 4.5) + ylim(-2.2, 1.8)
)
