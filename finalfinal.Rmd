...
blomqvist_boot_scaled <- function(data, indices, v_err_abs) {
  ...
  k <- 2 * v_err_abs                    # Var(x) = 1
  unname((b_obs + k) / (1 - k))
}

# Run bootstrap
set.seed(123)
boot_out <- boot(data = pdam, statistic = blomqvist_boot_scaled, R = 1000, v_err_abs = v_err_abs)
...
# how to apply simulation based inference to a simulation with true relationship - can we recover true with an unknown measurement error / practical advice on precision of measurement error
# e.g., take scenario 2.5 and take a subset of the data and recover the true slope again? w/ various estimates of measurement error? or v_err rel to v_pop
...
v_err_abs <- 0.0231325
var_x_abs <- var(pdam$log_init_obs, na.rm = TRUE)
# Note this is done with UNSCALED predictor
b_obs <- coef(lm(log_change_obs ~ log_init_obs, data = pdam))[2]
var_x_abs <- var(pdam$log_init_obs, na.rm = TRUE)
k <- 2 * v_err_abs / var_x_abs
b_true_blomqvist <- (b_obs + k) / (1 - k)

library(boot)

# Function to compute Blomqvist-corrected slope from a resampled dataset
blomqvist_boot <- function(data, indices, v_err_abs) {
  d <- data[indices, ]
  x <- d$log_init_obs
  y <- d$log_final_obs - d$log_init_obs
  b_obs <- coef(lm(y ~ x))[2]
...
# Run the bootstrap
set.seed(123)
boot_out <- boot(data = pdam, statistic = blomqvist_boot, R = 1000, v_err_abs = 0.0231325)

# Bootstrap percentile CI
boot_ci <- boot.ci(boot_out, type = "perc")

# Output
boot_out$t0              # Point estimate
boot_ci$percent[4:5]     # 95% CI
