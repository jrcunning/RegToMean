...
pdam_var_y_abs_low  <- pdam_var_boot_ci[1]
pdam_var_y_abs_high <- pdam_var_boot_ci[2]

pdam_var_y_rel_low  <- pdam_var_y_abs_low  / pdam_v_pop_abs
pdam_var_y_rel_high <- pdam_var_y_abs_high / pdam_v_pop_abs
...
# --- Step 1: Pivot simulation results to wide format ---
# Step 1: Filter simulations
...
# Unweighted b_sim ± CI (uniform over filtered set)
pdam_b_sim <- mean(pdam_sim_filtered$b)
pdam_b_sim_ci <- quantile(pdam_sim_filtered$b, c(0.025, 0.975), na.rm = TRUE)

# Step 2: Compute standard deviation of errors in slope and variance (from observed) across simulations
...
# Step 3: Apply weights (standardized to units of standard deviation)
...
hist(pdam_sim_wtd$weight, breaks = 100)

# Step 4: Compute weighted summary
...
cat("Unweighted CI width:", diff(pdam_b_sim_ci), "\n")
cat("Weighted CI width:", diff(pdam_b_sim_ci_wtd), "\n")
...
k <- 2 * pdam_v_err_rel                   
pdam_b_blomq <- unname((pdam_b_obs + k) / (1 - k))  # unname to remove confusion
...
mill_pars <- expand_grid(
  ...
  v_err = v_err_rel, #seq(0, 0.05, 0.01),        # Meas. err. variance on rel. scale (95% CI of v_err_abs / v_pop_abs)
  b = seq(-1, 1, 0.01)                  # Test all unknown true relationships
)
...
evaluate_methods <- function(b_true, v_pop, v_ind, v_err, seed, n = 200, reps = 50) {
  replicate_results <- purrr::map_dfr(seq_len(reps), function(i) {
    ...
    # # Step 2: Compute standard deviation of errors in slope and variance (from observed) across simulations
    # slope_error_sd <- sd(sim_filtered$est - b_obs, na.rm = TRUE)
    # var_error_sd   <- sd(sim_filtered$var_sim - var_y_rel, na.rm = TRUE)

    # Compute weights based on scaled errors
    # Step 3: Apply weights (standardized to units of standard deviation)
    # sharpness <- 100  # Tune this as needed
    # sim_wtd <- sim_filtered %>%
    #   mutate(
    #     slope_error_z = (est - b_obs) / slope_error_sd,
    #     var_error_z   = (var_sim - var_y_rel) / var_error_sd,
    #     weight        = exp(-0.5 * sharpness * (slope_error_z^2 + var_error_z^2))
    #   )
    # 
    # # Step 4: Compute weighted summary
    # b_sim_wtd <- Hmisc::wtd.mean(
    #   sim_wtd$b,
    #   weights = sim_wtd$weight,
    #   normwt = TRUE
    # )
  ...
  summarised <- replicate_results %>%
    summarise(across(
      .cols = c(b_obs, b_blomq, b_kp, b_sim),#, b_sim_wtd),
      .fns = list(
...
results_long <- results_all %>%
  pivot_longer(
    cols = c(b_obs_mean, b_kp_mean, b_blomq_mean, b_sim_mean),# b_sim_wtd_mean),
    names_to = "method",
    values_to = "estimate"
  ) %>%
  mutate(
    sd = case_when(
      ...
      #method == "b_sim_wtd_mean"~ b_sim_wtd_sd
    ),
