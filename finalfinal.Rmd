...
mill_pars <- expand_grid(
  v_pop = 1,                            # Set trait variance to 1 (relative)
  v_ind = seq(0, 1, 0.025),               # Test broad v_ind (relative to v_pop = 1; 0.23 estimate - mill)
...
param_grid <- expand.grid(
  ...
  v_ind = c(0.01, 0.1, 0.5, 1, 2),
  v_err = c(0.01, 0.05, 0.1, 0.2, 0.4),
  v_pop = 1
) %>%
  dplyr::mutate(seed = sample.int(1e6, size = dplyr::n(), replace = FALSE))


evaluate_methods <- function(b_true, v_pop, v_ind, v_err, seed, n = 1000) {
  set.seed(seed)
  ...
  # Step 5: ABC-like inference
  boot_var <- boot(syn_df, function(data, i) var(data[i, ]$log_change_obs), R = 1000)
  var_y <- var(syn_df$log_change_obs)
  var_ci <- quantile(boot_var$t, c(0.025, 0.975))
  
  sim_pars <- expand.grid(
    v_ind = seq(0, 4, 0.25),
    b = seq(-1, 1, 0.05),
    v_err = v_err, v_pop = v_pop
  )
  ...
  sim_results <- pmap_dfr(sim_pars, test.slopes)
  
  slope_ci <- coef(summary(obs_fit))["scale(log_init_obs)", "Estimate"]
  ...
  b_sim_mean <- mean(sim_filtered$b, na.rm = TRUE)
  
  # Return all slope estimates + true b
  tibble(
    b_true = b_true,
    b_obs = b_obs,
    b_blomq = b_blomq,
...
results_long <- results_all %>%
  pivot_longer(
    cols = c(b_obs, b_kp, b_blomq, b_sim),  # exclude b_true
    names_to = "method",
    values_to = "estimate"
  ) %>%
  mutate(error = estimate - b_true)
...
ggplot(results_long, aes(x = b_true, y = error, color = method, shape = method)) +
  ...
  labs(y = "Error in estimated slope", x = "True slope (b)") +
  coord_cartesian(ylim = c(-1, 1))


results_long %>%
  group_by(method) %>%
  summarize(mean_error = mean(abs(error), na.rm = TRUE))
```
