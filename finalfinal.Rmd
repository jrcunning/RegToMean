...
results %>%
  ...
  filter(cor.type %in% c("obs.b", "obs.b2", "adj.b")) %>%
...
### Simulation-based inference of true relationship given observed slope and total variance in *P. damicornis* study, and known measurement error

```{r pdam_scenarios}
# I believe this is essentially an "ABC-rejection algorithm" (approximate bayesian computation)
## Likelihood-free inference via Approximate Bayesian Computation (ABC)
## Specifically, it’s ABC with rejection sampling and uniform priors over your parameter grid.
...
pdam_pars <- expand_grid(
  ...
  v_ind = seq(0, 4, 0.5),               # Test broad v_ind (relative to v_pop = 1; Pdam estimate = 1.57)
  v_err = seq(0.06, 0.27, 0.1),        # Meas. err. variance on rel. scale (95% CI of v_err_abs / v_pop_abs)
                                           # this is from confint; see pdam_variance.R
  b = seq(-1, 1, 0.1)                  # Test all unknown true relationships
)
...
# "The observed slope and its 95% confidence interval reflect sampling variability in the relationship between observed change and initial trait value. The simulation-based interval reflects the range of true underlying slopes that could plausibly give rise to the observed slope and variance in change, given known measurement error. The contrast between these estimates highlights the possible influence of regression to the mean."
# df_plot <- tibble(
#   source = c("Observed", "Simulation-based"),
#   estimate = c(b_obs, mean(pdam_sim_possible$b)),
#   lower = c(b_obs_ci[1], quantile(pdam_sim_possible$b, 0.025)),
#   upper = c(b_obs_ci[2], quantile(pdam_sim_possible$b, 0.975))
# )
# 
# ggplot(df_plot, aes(x = source, y = estimate)) +
#   geom_point(size = 3) +
#   geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
#   geom_hline(yintercept = 0, linetype = "dashed") +
#   labs(y = "Slope (b)", x = NULL,
#        title = "Comparison of observed vs. inferred true slope")

```

```{r pdam_blomqvist}
# POINT ESTIMATE using scaled predictor
b_obs_scaled <- coef(lm(log_change_obs ~ scale(log_init_obs), data = pdam))[2]
k <- 2 * v_err_rel                   
b_true_blomqvist <- unname((b_obs_scaled + k) / (1 - k))  # unname to remove confusion

# BOOTSTRAP FUNCTION
blomqvist_boot_scaled <- function(data, indices, v_err_rel) {
  d <- data[indices, ]
  x <- scale(d$log_init_obs)[, 1]       # Standardized predictor
  y <- d$log_final_obs - d$log_init_obs # Observed change
  b_obs <- coef(lm(y ~ x))[2]           # Slope from scaled X
  k <- 2 * v_err_rel                    # Var(x) = 1
  unname((b_obs + k) / (1 - k))
}

# Run bootstrap
set.seed(123)
boot_out <- boot(data = pdam, statistic = blomqvist_boot_scaled, R = 1000, v_err_rel = v_err_rel)

# Bootstrap 95% percentile confidence interval
boot_ci <- boot.ci(boot_out, type = "perc")

# Output results
b_true_blomqvist           # Point estimate
boot_ci$percent[4:5]       # 95% CI
```

```{r pdam_compareslopes}
# # Step 1: Create a tidy data frame for the summary points
# b_summary <- tibble(
#   method = c("Observed", "Kelly-Price Adjusted", "Simulation-Inferred"),
#   estimate = c(b_obs,
#                mean(pdam_sim_possible$estimate_adj.b),
#                mean(pdam_sim_possible$b)),
#   lower = c(b_obs_ci[1],
#             quantile(pdam_sim_possible$estimate_adj.b, 0.025),
#             quantile(pdam_sim_possible$b, 0.025)),
#   upper = c(b_obs_ci[2],
#             quantile(pdam_sim_possible$estimate_adj.b, 0.975),
#             quantile(pdam_sim_possible$b, 0.975))
# )
# 
# # Step 2: Plot point + interval for all three methods
# ggplot(b_summary, aes(x = method, y = estimate)) +
#   geom_point(size = 3) +
#   geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
#   geom_hline(yintercept = 0, linetype = "dashed") +
#   labs(
...
# Step 2: Plot with correct top-to-bottom order
ggplot(b_summary, aes(x = method, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
v_err_abs <- 0.01301945     # on trait scale (logTLE; see "RegressionToMean_forGithub.R")
v_pop_abs <- var_x_abs - v_err_abs       # Initial true trait value variance (subtract measurement error var)
v_err_rel <- v_err_abs / v_pop_abs       # Measurement error variance on relative scale
                                         # i.e., relative to v_pop since v_pop standardized to 1 in simulations
...
mill_pars <- expand_grid(
  v_pop = 1,                            # Set trait variance to 1 (relative)
  v_ind = seq(0, 1, 0.1),               # Test broad v_ind (relative to v_pop = 1; 0.23 estimate - mill)
  v_err = seq(0, 0.05, 0.01),        # Meas. err. variance on rel. scale (95% CI of v_err_abs / v_pop_abs)
...
# df_plot <- tibble(
#   source = c("Observed", "Simulation-based"),
#   estimate = c(b_obs, mean(mill_sim_possible$b)),
#   lower = c(b_obs_ci[1], quantile(mill_sim_possible$b, 0.025)),
#   upper = c(b_obs_ci[2], quantile(mill_sim_possible$b, 0.975))
# )
# 
# ggplot(df_plot, aes(x = source, y = estimate)) +
#   geom_point(size = 3) +
...
b_summary <- tibble(
  method = c("Observed", "Kelly-Price Adjusted", "Simulation-Inferred"),
  estimate = c(b_obs,
               mean(mill_sim_possible$estimate_adj.b),
               mean(mill_sim_possible$b)),
  lower = c(b_obs_ci[1],
            quantile(mill_sim_possible$estimate_adj.b, 0.025),
            quantile(mill_sim_possible$b, 0.025)),
  upper = c(b_obs_ci[2],
            quantile(mill_sim_possible$estimate_adj.b, 0.975),
            quantile(mill_sim_possible$b, 0.975))
)
...
```{r synthetic_dataset}
# --- STEP 1: Simulate synthetic dataset with known true parameters ---
set.seed(123)
n <- 100
b_true <- -0.4
v_pop <- 1
...
# Create synthetic dataset based on set parameters
...
# Kelly Price adjustment
syn_df <- syn_df %>% mutate(
  log_change_obs_adj = rttm.adj(log_init_obs, log_final_obs)[, 3]
)

# Plot synthetic dataset
syn_initfinal <- range(c(syn_df$log_init_obs, syn_df$log_final_obs), na.rm = TRUE)
syn_initchange <- range(c(syn_df$log_init_obs, syn_df$log_change_obs), na.rm = TRUE)

synthetic_plots <- list(
  chiolero_plot(syn_df, log_init_obs, log_final_obs),
  p6 %+% syn_df + coord_fixed(xlim = syn_initfinal, ylim = syn_initfinal),
  p7 %+% syn_df + coord_fixed(xlim = syn_initfinal, ylim = syn_initchange),
  p8 %+% syn_df + coord_fixed(xlim = syn_initfinal, ylim = syn_initchange)
)

# # Assemble the 4-panel row
# wrap_plots(synthetic_plots, nrow = 1) +
#   plot_annotation(tag_levels = "A") &
#   theme(plot.tag = element_text(size = 12, face = "bold"))
# 
# # Add top annotations
...
# --- STEP 2: Estimate observed slope and variance (absolute and relative scales) ---
obs_fit <- lm(log_change_obs ~ scale(log_init_obs), data = syn_df)
b_obs <- coef(obs_fit)[2]
b_obs_ci <- confint(obs_fit)[2, ]
var_y_abs <- var(syn_df$log_change_obs)
v_err_rel <- v_err / v_pop
var_y_rel <- var_y_abs / v_pop

# --- STEP 3: Bootstrap CI for variance in change (to constrain simulations) ---
boot_fun <- function(data, indices) var(data[indices, ]$log_change_obs)
boot_out <- boot(syn_df, boot_fun, R = 1000)
boot_ci <- boot.ci(boot_out, type = "perc")
var_y_rel_low  <- quantile(boot_out$t, 0.025)
var_y_rel_high <- quantile(boot_out$t, 0.975)

# --- STEP 4: Simulation grid across parameter space ---
sim_pars <- expand_grid(
  v_pop = 1,
  v_ind = seq(0, 5, 0.1),
  v_err = v_err, #seq(0.05, 0.25, 0.02),  # decide if just use point estimate of error, or add uncertainty... blomqvist just uses a point estimate?
  b = seq(-1, 1, 0.005)
)

# --- STEP 5: Run simulations across grid ---
test.slopes <- function(v_pop, v_ind, v_err, b) {
  ...
  # Get true, observed, and adjusted correlations between change and initial
  bind_rows(obs.b = tidy(lm(log_change_obs ~ scale(log_init_obs), data = df))[2,],
  ) %>% bind_cols(
    v_pop = v_pop,
    v_ind = v_ind,
    v_err = v_err,
...
# --- STEP 6: Filter simulations consistent with slope and variance observed ---
# All slopes are computed using standardized predictors (Var(X) = 1), so results are on a comparable scale
sim_possible <- sim_results %>%
  select(obs.b = estimate, everything()) %>%
  filter(
    obs.b > b_obs_ci[1] & obs.b < b_obs_ci[2],
    (v_ind + b^2 + 2 * v_err >= var_y_rel_low) &
    (v_ind + b^2 + 2 * v_err <= var_y_rel_high)
  )

# --- STEP 7: Simulation-inferred slope summary ---
b_sim_mean <- mean(sim_possible$b)
b_sim_ci <- quantile(sim_possible$b, c(0.025, 0.975))



# --- STEP 8: Blomqvist correction with bootstrap CI ---
k <- 2 * v_err_rel
b_true_blomqvist <- unname((b_obs + k) / (1 - k))

# Bootstrap function (recomputes Blomqvist-corrected slope from resampled data)
blomqvist_boot <- function(data, indices) {
  d <- data[indices, ]
...
# Run bootstrap
set.seed(123)
boot_out <- boot(data = syn_df, statistic = blomqvist_boot, R = 1000)

# Use quantiles directly from bootstrap distribution for CI
b_blomq_ci <- quantile(boot_out$t, c(0.025, 0.975))

# --- STEP 9: Kelly-Price adjusted slope ---
kp_fit <- lm(log_change_obs_adj ~ scale(log_init_obs), data = syn_df)
b_kp <- coef(kp_fit)[2]
b_kp_ci <- confint(kp_fit)[2, ]



# --- STEP 10: Compare slope estimates ---
b_summary <- tibble(
  method = c("Observed", "Blomqvist Corrected", "Kelly-Price Adjusted", "Simulation-Inferred"),
  estimate = c(b_obs, b_true_blomqvist, b_kp, b_sim_mean),
  lower = c(b_obs_ci[1], b_blomq_ci[1], b_kp_ci[1], b_sim_ci[1]),
  upper = c(b_obs_ci[2], b_blomq_ci[2], b_kp_ci[2], b_sim_ci[2])
)

ggplot(b_summary, aes(x = method, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_hline(yintercept = b_true, linetype = "dashed", color = "blue") +
  annotate("text", x = 4.5, y = b_true, label = "True b", hjust = 1, vjust = -0.5,
           color = "blue", fontface = "italic") +
  labs(
    title = "Comparison of Slope Estimates vs. True b (Synthetic Dataset)",
    y = "Slope (b)", x = NULL
  ) +
  theme_minimal(base_size = 14) +
  coord_flip()
...
param_grid <- expand.grid(
  ...
  v_ind = c(0.01, 0.25, 1, 4),
  v_err = c(0.01, 0.1, 0.2, 0.4),
  v_pop = 1
) %>%
...
# Test with replicate datasets for each parameter set
# Updated seeds for replicates per parameter set
evaluate_methods <- function(b_true, v_pop, v_ind, v_err, seed, n = 200, reps = 50) {
  replicate_results <- purrr::map_dfr(seq_len(reps), function(i) {
    ...
    # Step 2: Observed slope
    obs_fit <- lm(log_change_obs ~ scale(log_init_obs), data = syn_df)
    b_obs <- coef(obs_fit)[2]

    # Step 3: Blomqvist correction
    v_err_rel <- v_err / v_pop
    k <- 2 * v_err_rel
    b_blomq <- unname((b_obs + k) / (1 - k))

    # Step 4: Kelly-Price adjustment
    kp_fit <- lm(log_change_obs_adj ~ scale(log_init_obs), data = syn_df)
    b_kp <- coef(kp_fit)[2]
    ...
    test.slopes <- function(v_pop, v_ind, v_err, b) {
      x <- rnorm(n, 0, sqrt(v_pop))
      change <- b * scale(x)[, 1] + rnorm(n, 0, sqrt(v_ind))
      final <- x + change
      e1 <- rnorm(n, 0, sqrt(v_err))
      e2 <- rnorm(n, 0, sqrt(v_err))
      x_obs <- x + e1
      y_obs <- final + e2
    ...
    sim_filtered <- sim_results %>%
      ...
      filter(v_ind + b^2 + 2 * v_err >= var_ci[1],
             v_ind + b^2 + 2 * v_err <= var_ci[2])
  ...
  summarised <- replicate_results %>%
    summarise(across(
      ...
      .fns = list(
        ...
        sd   = ~ sd(.x, na.rm = TRUE),
        mae  = ~ mean(abs(.x - b_true), na.rm = TRUE)
      ),
      .names = "{.col}_{.fn}"
    )) %>%
    mutate(
...
plan(multisession)
results_all <- future_pmap_dfr(param_grid, evaluate_methods)
...
# ggplot(results_long, aes(x = b_true, y = error, color = method, shape = method)) +
#   #geom_point(size = 2, alpha = 0.8) +
#   geom_errorbar(aes(ymin = error - sd, ymax = error + sd), width = 0.05, alpha = 0.6) +
#   geom_line(aes(group = method), linewidth = 0.6, alpha = 0.7) +
...
#     x = "True slope (b)",
#     title = "Comparison of Slope Estimates Across Methods and Parameter Space"
#   ) +
#   theme_minimal(base_size = 14) +
#   coord_cartesian(ylim = c(-0.5, 0.5))
# 
# 
# write_csv(results_long, "results_long.csv")

# Prepare data
results_plot <- results_long %>%
  mutate(
    shape = case_when(
      error > 0.4  ~ 2,  # triangle up
      error < -0.4 ~ 6,  # triangle down
      TRUE         ~ 16   # circle
    ),
    error_clipped = pmax(pmin(error, 0.4), -0.4)
  )

# Plot
ggplot(results_plot, aes(x = b_true, y = error_clipped, color = method, group = method)) +
  geom_line(linewidth = 0.5, alpha = 0.6) +
  geom_errorbar(aes(ymin = error - sd, ymax = error + sd), width = 0.05, alpha = 0.6) +
  geom_point(aes(shape = factor(shape)), size = 2, fill = "white") +
  scale_shape_manual(values = c("2" = 2, "6" = 6, "16" = 16), guide = "none") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  facet_grid(v_ind ~ v_err, labeller = label_both) +
  coord_cartesian(ylim = c(-0.4, 0.4)) +
  labs(x = "True slope (b)", y = "Slope error (estimate - true)", color = "Method") +
  theme_minimal(base_size = 14)
...
ggplot(results_long, aes(x = method, y = error, fill = method)) +
  geom_violin(trim = TRUE, scale = "area", width = 5) +
  #geom_boxplot(width = 0.1, color = "black", fill = "white") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(y = "Slope error", x = NULL) +
  theme_minimal() +
...
bring in another dataset from Kelly and Price that was dismissed as RTM