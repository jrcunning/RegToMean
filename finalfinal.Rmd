...
ggplot(plot_df, aes(x = method, y = mean)) +
  ...
  annotate("text", x = 4.5, y = test_syn_eval$b_true, label = "True b", hjust = 1,
...
param_grid <- expand.grid(
  ...
  n = 200,                                          # size of each synthetic dataset (should represent 'typical' scientific study)
...
syn_df <- tibble(
  log_init_true = rnorm(n, 0, sqrt(syn_v_pop)),
  log_change_true = b_true * scale(log_init_true)[, 1] + rnorm(n, 0, sqrt(syn_v_ind)),
  log_final_true = log_init_true + log_change_true,
  syn_e_init = rnorm(n, 0, sqrt(syn_v_err)),
  syn_e_final = rnorm(n, 0, sqrt(syn_v_err)),
  log_init_obs = log_init_true + syn_e_init,
  log_final_obs = log_final_true + syn_e_final,
  log_change_obs = log_final_obs - log_init_obs
)
...
# ---------------------------
# --- 1. Observed slope and variance (absolute and relative scales) ---
syn_mod <- lm(log_change_obs ~ scale(log_init_obs), data = syn_df)
syn_b_obs <- coef(syn_mod)[[2]]
syn_b_obs_ci <- confint(syn_mod)[2, ]
syn_var_y_abs <- var(syn_df$log_change_obs)
syn_v_err_rel <- syn_v_err / syn_v_pop
syn_var_y_rel <- syn_var_y_abs / syn_v_pop
# --- Bootstrap CI for variance in change (to constrain simulations) ---
syn_var_y_boot_fun <- function(data, indices) var(data[indices, ]$log_change_obs)
syn_var_y_boot_out <- boot(syn_df, syn_var_y_boot_fun, R = 1000)
syn_var_y_boot_ci <- boot.ci(syn_var_y_boot_out, type = "perc")
syn_var_y_rel_low  <- quantile(syn_var_y_boot_out$t, 0.025)
syn_var_y_rel_high <- quantile(syn_var_y_boot_out$t, 0.975)



# ---------------------------
# 2. Kelly-Price adjusted slope and C.I.
syn_kp_mod <- lm(log_change_obs_adj ~ scale(log_init_obs), data = syn_df)
syn_b_kp <- coef(syn_kp_mod)[[2]]
syn_b_kp_ci <- confint(syn_kp_mod)[2, ]



# ---------------------------
# 3. Blomqvist adjusted slope and C.I.
# --- STEP 8: Blomqvist correction with bootstrap CI ---
syn_k <- 2 * syn_v_err_rel
syn_b_blomq <- (syn_b_obs + syn_k) / (1 - syn_k)
# Bootstrap function (recomputes Blomqvist-corrected slope from resampled data)
syn_blomqvist_boot <- function(data, indices) {
  d <- data[indices, ]
  b_obs_i <- coef(lm(log_change_obs ~ scale(log_init_obs), data = d))[[2]]
  (b_obs_i + syn_k) / (1 - syn_k)
}
# Run bootstrap
set.seed(123)
syn_b_blomq_boot_out <- boot(data = syn_df, statistic = syn_blomqvist_boot, R = 1000)
# Use quantiles directly from bootstrap distribution for CI
syn_b_blomq_ci <- quantile(syn_b_blomq_boot_out$t, c(0.025, 0.975))


# ---------------------------
# 4. Simulation-inferred slope
# --- Simulation grid across parameter space ---
syn_sim_pars <- expand_grid(
  n = 10000,
  v_pop = 1,
  v_ind = seq(0, 3, 0.1),
  v_err = syn_v_err_rel,
...
# ---  Run simulations across grid ---
test.slopes <- function(v_pop, v_ind, v_err, b, n) {
  # Simulate data
  df <- tibble(
    log_init_true = rnorm(n, mean = 0, sd = sqrt(v_pop)),
    log_change_true = b * scale(log_init_true)[,1] + rnorm(n, mean = 0, sd = sqrt(v_ind)),   
  ...
  # Get true and observed correlations between change and initial
...
plan(multisession)
syn_sim_results <- future_pmap_dfr(
  syn_sim_pars,
  test.slopes,
  .options = furrr_options(seed = TRUE, packages = "broom")
)

## --- Filter simulations consistent with slope and variance observed ---
# All slopes are computed using standardized predictors (Var(X) = 1), so results are on a comparable scale
syn_sim_filtered <- syn_sim_results %>%
  select(obs.b = estimate, everything()) %>%
  filter(
    obs.b > syn_b_obs_ci[1] & obs.b < syn_b_obs_ci[2],
    (v_ind + b^2 + 2 * v_err >= syn_var_y_rel_low) &
    (v_ind + b^2 + 2 * v_err <= syn_var_y_rel_high)
  )

## --- Simulation-inferred slope summary ---
syn_b_sim <- mean(syn_sim_filtered$b)
syn_b_sim_ci <- quantile(syn_sim_filtered$b, c(0.025, 0.975))




# PLOT
# --- Compare slope estimates ---
syn_b_summary <- tibble(
  method = c("Observed", "Blomqvist Corrected", "Kelly-Price Adjusted", "Simulation-Inferred"),
  estimate = c(syn_b_obs, syn_b_blomq, syn_b_kp, syn_b_sim),
  lower = c(syn_b_obs_ci[1], syn_b_blomq_ci[1], syn_b_kp_ci[1], syn_b_sim_ci[1]),
  upper = c(syn_b_obs_ci[2], syn_b_blomq_ci[2], syn_b_kp_ci[2], syn_b_sim_ci[2])
)

ggplot(syn_b_summary, aes(x = method, y = estimate)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
...
```{r old_parameter grid search synthetic }
set.seed(999)  # for reproducibility of seeds in parameter grid
